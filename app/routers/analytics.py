...
Provide driver IDs as repeated query parameters, for example
`?driver_ids=4&driver_ids=20&driver_ids=5`.
...
```
GET /api/v1/analytics/drivers/compare?driver_ids=4&driver_ids=20&driver_ids=5
```
    """,
    tags=["Analytics"],
)
async def compare_drivers(
    driver_ids: List[int] = Query(description="Repeat this query parameter for 2-5 driver IDs"),
    db: AsyncSession = Depends(get_db),
):
